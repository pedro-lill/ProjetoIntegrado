<?php
    ...
    function montaCalendario($eventos = array()){
        ...
        for($i =1; $i <= 12; $i++){
            ...
            for($n=1; $n<= $diasMeses[$i]; $n++){//percorre os dias e meses
                $dayMonth = gregoriantojd($i, $n, date('Y'));   //gregorian para julianday(diasemana por extenso), retorno o dia da semana na data seleionada
                $weekMonth = substr(jddayofweek($dayMonth, 1),0,3); //o 1 é pra retornar por extenso diaSemana. o substr (0, 3) exibe os 3 caracteres iniciais
                if($weekMonth == 'Mun') $weekMonth = 'Mon';//o retorno da jddayofweek é munday e nao monday, o if concerta isso
